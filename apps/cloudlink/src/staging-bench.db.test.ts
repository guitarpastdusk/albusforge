...
import { BenchJournal, createBenchSql, cleanupBenchSql } from './staging-bench.js';
...
it('creates only a bench device with exact camera capability; cleanup retains intents and fences recreation', async () => {
 const j=journal(); await pool.query(createBenchSql(j,'c'.repeat(64)));
 expect((await pool.query('SELECT interval_s,max_bytes,max_width,max_height FROM telemetry.device_capabilities WHERE device_id=$1',[j.device])).rows).toEqual([{interval_s:900,max_bytes:1048576,max_width:320,max_height:240}]);
