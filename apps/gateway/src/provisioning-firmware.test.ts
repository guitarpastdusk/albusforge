...
function fixture() {
  ...
  const record: FirmwarePassedRecord = { schema_version: 1, candidate_id: manifest.profile_id, manifest, manifest_digest: createHash("sha256").update(serializeFirmwareManifest(manifest)).digest("hex"), compiler: { image_digest: `espressif/idf@sha256:${"c".repeat(64)}`, idf_version: "5.5.3" }, source_sha256: "d".repeat(64), instruction: "test fixture only", diagnostics: "test fixture only; not hardware acceptance" };
...
it("rejects modified manifest bytes, missing evidence and unexpected persisted fields", () => {
  ...
  expect(verifyProvisioningFirmware({ ...record, candidate_id: "other" }, identity)).toBeNull();
