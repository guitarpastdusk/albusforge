...
1. **Lock.** `pg_try_advisory_lock(424200001, hashtext(build_id))` on the turn's connection. A caller that can't take it returns `noop`: the holder answers. The holder checks for an unanswered message again after unlocking, so a message sent mid-turn is picked up rather than stranded (at most three turns per call).
   ...
   The whole call is spent against one **budget**: `TURN_DEADLINE_MS` for the model plus `CLEANUP_RESERVE_MS` (3 s) for cleanup and the write, and it has to end inside gateway's 50 s attempt deadline, because gateway does not retry its own timeout — a turn it stopped waiting for may still be running. Startup refuses a `TURN_DEADLINE_MS` that wouldn't leave room. Every step that can block gets what the budget has left rather than a timeout of its own: the model deadline is shortened if the reads were slow, cleanup is bounded, and so is the final write. When the budget runs out, every handle is fenced in one sweep and the connection is destroyed rather than asked one more question — that also releases the advisory lock, so gateway's retry finds the build free instead of a noop from a lock nobody is using. The failure is a typed `DatabaseUnavailableError`, so the route answers `503` and the retry actually runs.

   Sharing one connection means the deadline needs a cleanup path, not just a fence. The deadline can land inside metering's transaction, which takes the build row's lock and so can be held up by ordinary contention; the reply is written on that same connection. Before the write, the turn's handle is closed: new work stops, outstanding queries are drained, and the transaction is rolled back from here, with Drizzle's own rollback fenced out so the two can't race. Without that, an abandoned statement hits `statement_timeout`, leaves its transaction aborted, and the write's `BEGIN` fails with 25P02 — no reply at all. If the connection can't be left clean it is destroyed rather than reused and the turn is a `503`; the guarantee then rests on gateway's retry, which answers the message once.
...
| `TURN_DEADLINE_MS` | `45000` | the model call, retries included. Plus a 3 s cleanup reserve, this is the whole call's budget, and it must fit inside gateway's 50 s attempt deadline |
...
That guarantee is enforced at the logger, not by convention: an error is serialized to its class, its `code`/`status` and its stack **frames**, and its message is dropped unless the error is marked `safeToLog` (config and startup failures, which are operator-authored). An error message can carry anything the code that threw it had in hand — an excerpt of a model response, a row, a person's words. The frames are parsed by removing V8's exact `name: message` header and then requiring every remaining line to be a frame, because V8 keeps the whole message inside `stack`: a multi-line message can otherwise write its own `at …` line. An unrecognized stack format yields no frames rather than guessed ones. Failed calls log `callStructured`'s sanitized diagnostic instead: a failure class, schema paths and issue codes from our own schema, and the provider's error class, HTTP status and request id. `test/redaction.test.ts` drives each of these paths with a synthetic private marker and asserts it appears in no log field.
...
- `src/handler.db.test.ts`, against Postgres:
  ...
  - the deadline landing inside metering's transaction while another connection holds the build row's lock, with the statement then timing out: the fallback reply is still written and the usage row is rolled back
  - a connection that goes quiet after the model answers, against the real gateway scheduler: intake's 503 arrives inside gateway's attempt deadline and the retry runs, with no cleanup statement sent to the dead connection and the advisory lock gone with it
...
- `test/redaction.test.ts`: the real failure-to-logger path for a JSON parse failure, a schema mismatch, a provider error, a failed usage insert and a database message whose own newlines imitate stack frames, each carrying a private marker that must not reach a log
