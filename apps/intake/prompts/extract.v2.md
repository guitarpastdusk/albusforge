...
- `candidate_questions`: the question for the person, tied to the one spec field its answer resolves, such as `power.source`. Give `options` when the answer is a small closed set — two to four of them, each a short phrase the person could have typed themselves (`["USB power", "Battery"]`, `["Indoors", "Outdoors"]`). The person taps one and it is sent as their reply, so write them as answers, not as labels: "Battery", not "battery option". Leave `options` out when the answer is open-ended — how many days it must last, where it will live, what it should measure — because a guessed list is worse than a typed answer.
