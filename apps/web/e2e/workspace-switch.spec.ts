...
test("workspace switch clears both tabs, isolates another session, and reconciles uncertain failure", async ({ page, context, browser, stack }, info) => {
  ...
  await page.reload();await expect(page.getByText("Only in Workspace A", { exact: true })).toBeVisible();
  ...
  try {
    ...
    const separate=await independent.newPage();await signIn(separate, stack, email);await expect(separate.getByText("Only in Workspace A", { exact: true })).toBeVisible();
    ...
    await expect(peer.getByText("Only in Workspace A",{exact:true})).toHaveCount(0);
    release();
    for(const tab of [page,peer]){await expect(tab.getByText("Only in Workspace B",{exact:true})).toBeVisible();await expect(tab.getByText("Only in Workspace A",{exact:true})).toHaveCount(0);}
    await expect(separate.getByText("Only in Workspace A",{exact:true})).toBeVisible();
    ...
    await expect(page.getByText("Only in Workspace B",{exact:true})).toHaveCount(0);
    await page.getByRole("button",{name:"Reload workspace"}).click();
    await expect(page.getByText("Only in Workspace B",{exact:true})).toBeVisible();
