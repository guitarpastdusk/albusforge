...
function PlanCard({plan,current,canAccept,pending,accept}:{plan:PersistedBuildPlan;current:number|null;canAccept:boolean;pending:boolean;accept:()=>void}){
  ...
  return <section className="min-w-0 rounded-[24px] border border-hairline bg-white p-6" aria-labelledby={`plan-${plan.version}`}>
    ...
    <div className="mt-5 overflow-x-auto"><table className="w-full text-left text-sm"><caption className="mb-3 text-left font-semibold">Bill of materials</caption><thead><tr><th scope="col" className="py-2 pr-4">Part and version</th><th scope="col" className="px-2">Quantity</th><th scope="col" className="pl-2">Unit estimate</th></tr></thead><tbody>{plan.plan.bom.map(item=><tr key={`${item.part.id}@${item.part.version}`} className="border-t border-hairline"><th scope="row" className="py-3 pr-4 font-normal">{item.part.id}@{item.part.version}</th><td className="px-2">{item.quantity}</td><td className="pl-2">{cost(item.unit_cost_usd)}</td></tr>)}</tbody></table></div>
