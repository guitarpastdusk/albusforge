...
- `ClosedLoopActions` — the "Closed loop · actions" card: kind pill, rule, "via" line and a `Toggle` per action, plus the "+ New action" footer. **Interactive only in mock mode** (a local simulation). In live mode it is read-only — switches and "+ New action" disabled, a "Not connected yet" badge, and a note explaining why — because there is no API route for actions yet (TODO in the component and schema).
...
Ages are formatted with `now` from the server render, so they're deterministic. Tests in `devices.test.tsx`, `dashboard.test.tsx` and `device-chat.test.ts`.
