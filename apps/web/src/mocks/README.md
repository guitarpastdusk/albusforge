...
- `stream.ts` — `mockTenantStream`: the tenant SSE stream for local dev, served by `app/v1/tenants/[tenantId]/stream/route.ts` in mock mode. Replays a `status` event per online device on connect, then walks each online tile's channel by a small random step every 3 s and emits `reading` events.
