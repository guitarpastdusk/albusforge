...
**Sensor cloud rollout, 2026-09-13:** ingestion/storage/read APIs, telemetry monitor, bounded Ask service, authenticated gateway/portal integration and sensor infrastructure are merged. Staging infrastructure has been provisioned and converged; gateway/schema, intake, cloudlink, Ask, processing jobs and web releases at `03f40c1` succeeded. Deployed simulator ingestion, model-free Ask with zero-cost audit, post-ingest rollup, bounded HTTP load and polling-dashboard/chat browser checks passed. The durable quota SQL audit/cleanup and one real-provider Haiku call with attributed usage also passed. Staging model calls and schedules are enabled; three automatic rollups completed with healthy snapshots, fixture credentials were revoked/deleted, and active Terraform configuration converged with no changes. Automatic daily maintenance remains a separate observation. A user-authorized synthetic notification test passed incident/recovery delivery, both emails were confirmed received, and temporary resources were removed. Backend telemetry SSE remains unavailable. All six production runtime promotions succeeded; model/schedule activation applied, one production provider call passed SQL/log accounting, and browser checks passed. Production automatic rollups passed with healthy snapshots, fixture credentials were removed, and active configuration converged. Final inventory verifies certified images and 100% traffic to the ready revisions in both environments. Two inconsistent zero-job GitHub run records remain unresolved under a bounded cancellation watcher; workflow restoration is pending the documentation merge. [SENSOR-CLOUD-ROLLOUT.md](SENSOR-CLOUD-ROLLOUT.md) tracks ownership, observed deployed resources, dependency order and acceptance evidence. The initial Ask slice is a user-selected sensor/channel/window with a small hosted model interpreting bounded questions and deterministic code supplying the numerical answer. It does not implement the later cross-sensor agent, anomaly detectors or write tools.
...
- **The role is `infra/env` apply coordinator, with exactly one active holder.** Currently Claude session `albusforge-44`; the previous sensor-rollout holder, Codex thread `01a09b73-2e1d-7ef2-8d02-ce1639fc73ba`, transferred it on 2026-09-14 at 02:57 UTC and remains the fallback. Transfer must be explicit and acknowledged, or reassigned by the user when a holder is unavailable — nobody becomes a second owner merely because someone looks offline, and **this line is the roster: update it when ownership moves.** A message-only handover recreates exactly the discoverability problem this section exists to solve. Handover is a message, not a rebuild: the rollout var-files, plan/evidence workflow and `docs/SENSOR-INFRA.md` stay in place.
- **Always apply from merged `main`**, never a local branch, and **always pass the environment's var-file**: `-var-file=rollout-staging.tfvars.json` or `-var-file=rollout-prod.tfvars.json`. The defaults are not the deployed configuration. If the activation values are not supplied elsewhere, omitting the rollout file makes the plan propose pausing both schedules, disabling the five schedule-dependent alerts and the SQL connection alert, and removing Ask's model-key mount and accessor binding. Job-failure, HTTP-error, SQL CPU/disk, pool-wait and spend alerts are unaffected, and intake's own Anthropic binding remains. A plan run without the file *looks* like alarming drift when nothing is wrong.
- **Never leave configuration applied but uncommitted** — meaning nonsecret desired configuration, not secret values or private plan evidence. It is invisible to the next holder, and their next apply reverts it. PRs #51 and #67 are historical reconciliations of that gap, not a precedent that authorises repeating it.
- **Check for a held lock before planning**: `gs://albusforge-ci-tfstate/env/*.tflock`. A lock means another operation holds the state — plan takes it too, not only apply — or that a stale lock needs investigating. Inspect its operation and owner and coordinate; never pass `-lock=false` for anything that writes. Listing lock objects is a point-in-time check, so leave Terraform's own locking on, and note that the absence of a lock does not transfer ownership.
- Per [ADR 0005](adr/0005-ci-owns-images-terraform-owns-shape.md), disable and drain the deploy workflows before an apply that touches a service or job, pausing affected schedules, and **re-run any deploys that were skipped while they were off** — merges landing in that window are silently not deployed.
