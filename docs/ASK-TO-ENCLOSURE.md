...
- **Not ready until the API is reachable.** A cold Cloud Run instance passes its TCP startup probe as soon as the port opens, but egress to the internet isn't usable for some seconds after that — private-range traffic already is, so the database answers while the model API doesn't (ADR 0004). Intake and ask therefore wait for an unauthenticated probe of the API to answer before they listen, bounded and logged, so Cloud Run holds the first request instead of routing it into a turn that can only fail.
