...
- **Structured output, through one wrapper that owns every failure path.** `SpecTurn` is a Zod schema in `packages/schema` holding `{ reply_kind, spec_patch, candidate_questions[], assumptions[], reply }`, sent as `output_config.format` so intake never parses prose. The SDK's `messages.parse` **throws** when the text isn't valid JSON or fails the schema, rather than returning `parsed_output: null`, so intake doesn't call it directly. `packages/llm`'s `callStructured(route, schema, messages)` does this instead:
  ...
  4. **Validate** with `SpecTurn.safeParse` on the parsed text, catching JSON errors too. On failure there is one repair retry that includes the validation error, then a safe fallback reply with the spec unchanged.

  A request that never reached the API is retried on its own schedule (1 s, 3 s, 6 s) before any of this: the SDK's own retries all land inside about two seconds, which is too fast to outlast a route that isn't up yet. Those attempts cost no call, because nothing was answered.

  Each branch has a recorded-fixture test: refusal, truncation, invalid JSON, schema mismatch, repair success, and an unreachable API.
- **Staying on the build.** `reply_kind` marks a message that isn't about building a device at all. Intake writes that reply itself, so the model's prose can't wander into answering the question, and the turn writes no spec version and spends no clarification round. A second stray message in a row gets a closing reply rather than the same invitation again. An empty `spec_patch` can't stand in for this: a turn that only chats about the build in progress produces one too.
- **The cold-start egress window, measured.** A cold Cloud Run instance passes its TCP startup probe as soon as the port opens, but egress to the internet isn't usable for some seconds after that — private-range traffic already is, so the database answers while the model API doesn't (ADR 0004). Intake and ask time that window with an unauthenticated probe of the API, started after they listen and never awaited. Waiting for it before listening was tried and removed: on staging it timed out for its whole 28 s budget and the instance served anyway, so it cost every cold start 28 s and prevented nothing. A turn that arrives inside the window is carried by the retries instead — transport retries in `callStructured`, the hand-back to gateway's retry, and the portal's chase — which together cover minutes rather than seconds.
...
- **Deadlines, retries and the SSE fallback.** Intake gives each turn a 45 s server deadline, including retries, and every user message always ends with exactly one assistant message. A failure another attempt could fix — the API unreachable, or an unexpected throw — is handed back instead of answered: `IntakeTurnRequest.may_retry` says the caller will try again, and intake then writes nothing and answers 503, so gateway's retry answers the message properly. The last attempt always writes, and a deadline or a truncated or schema-failing turn writes straight away, since neither is helped by repeating it. The fallback reply says the failure was ours and never asks the person to rephrase: their message was fine, and rephrasing sends them round the same loop believing it wasn't.

  `GET /v1/builds/:id/messages` is the source of truth. If the SSE stream drops, the portal reconnects with `Last-Event-ID` and refetches messages. If no reply has arrived 60 s after sending, the portal checks for one itself, and again 30 s and 60 s later — each check a refetch, **never a resend**, since a read of an unanswered transcript is what makes gateway start a fresh turn. Only once those are spent does it say so and offer the check by hand. `PostMessageRequest` gains a client-generated `client_message_id`, so an accidental double submit is idempotent, and sends carry a client-side timeout so a lost request fails visibly instead of hanging the chat.
...
- **The join.** `building --> ready` happens only when the code bundle **and** the body for the build's **current** plan have both passed (compile gate and lint gate). Whichever passes second makes the transition. Re-reading inside a transaction isn't enough at PostgreSQL's default Read Committed isolation: two completions could each see the other artifact as unfinished, both commit, and leave the build stuck in `building`. So the join is **serialized on the build row**:
  1. Every artifact completion runs one transaction that first takes `SELECT … FROM builds WHERE id = $1 FOR UPDATE`, **before** recording its own result.
  2. With the lock held, it records the artifact's status, then re-reads the build's current plan and both artifact statuses.
  3. It moves the build to `ready` only if both artifacts for the current plan have passed and the build is still `building`, so the transition happens exactly once.
  4. **Anything that changes the current plan** (spec edits, re-solves, manual retries) takes the same row lock, so a completion and a plan change can't interleave.

  **Concurrency acceptance tests** run against a real PostgreSQL instance: both artifacts completing at the same time (the build reaches `ready` exactly once), and a completion racing a spec edit (a stale result never makes the new plan `ready`).
...
| **M5.5: Viewer** *(new)* | — | Lazy-loaded 360° viewer (three.js), exploded view, part ghosts, PNG fallback | CORS rule on the bodies bucket for the portal origins; the storage origin in `connect-src` and `img-src` if a CSP is in place |
