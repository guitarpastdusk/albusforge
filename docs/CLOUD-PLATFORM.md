...
| **Tenant root** | tenant created at sign-up; orders and devices take it from the build ([ADR 0009](adr/0009-tenant-created-at-sign-up.md)). Superseded: tenant from `h(order)`, which can't hold builds that have no order | **blocking for M1.** Cheap now, a seven-service migration later |
