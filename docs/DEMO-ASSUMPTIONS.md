...
| Device-ready card "View a sample enclosure in 3D →" | Staged: the checked-in fixture GLB, labelled "Sample enclosure · not this build's", in mock and live mode alike since no build has a body | `src/components/enclosure/fixture.ts` (`enclosurePreviewFor(body)`) | `GET /v1/builds/:id/body` (ASK-TO-ENCLOSURE §6): pass its answer to `enclosurePreviewFor` |
