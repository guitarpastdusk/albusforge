...
Worker environment: existing database variables, `FIRMWARE_ARTIFACT_BUCKET`, `FIRMWARE_COMPILER_MODE=idf`, `FIRMWARE_TEMPLATE_DIR=/app/firmware/esp32s3`. Gateway environment: the same bucket and `FIRMWARE_JOB_RESOURCE=projects/PROJECT/locations/REGION/jobs/fwbuild`. Grant gateway object-read and job-execution permissions; grant worker immutable object-create plus its database access. Configure job egress to private PostgreSQL and required Google APIs. Gateway never invokes a local compiler. For explicit local tests only, use `FIRMWARE_ARTIFACT_DIR` and gateway `FIRMWARE_JOBS_ENABLED=1`; do not configure both bucket and directory. A scheduled job invocation must drain queued work and recover abandoned leases; one invocation handles one row. Size invocation cadence and task parallelism against the database connection budget. No Terraform, scheduler, IAM or deployment is applied by this change.
