...
One deadline covers reservation, evidence, model and completion. SQL acquisition races the shared deadline in addition to its separate 3-second queue/connect bound; statements have 5-second server and 6-second client bounds. Deadline cancellation destroys an in-flight SQL lease, while late acquired leases are destroyed exactly once without issuing SQL. Checked-out clients have a lease-scoped socket-error handler; broken connections cannot crash the process or return to the pool. Premature HTTP response closure after body receipt aborts the shared controller, while normal completed responses do not. Provider cancellation also has a local race guard, so an ignored abort cannot retain an HTTP concurrency slot. An aborted request can fail with a sanitized 503 instead of an answer; reservations persist. The instance semaphore rejects excess work with 429. Cloud Run concurrency and pool limits should match the infrastructure budget.
