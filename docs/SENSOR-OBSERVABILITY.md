...
| --- | --- | --- |
| `telemetry_dirty_hours` | Exact number of dirty hour markers in a post-rollup database snapshot | p99 bucket estimate above 10,000 for 10m |
| `telemetry_oldest_dirty_seconds` | Age of earliest marker creation; zero for an empty queue, clamped at zero for future clock values | p99 bucket estimate above 900s for 10m |
...
| `ingest_pool_wait_ms` | Measured `pool.connect()` elapsed time, including queue and connection setup; acquired/failed outcome | p95 above 500ms for 5m |
...
Telemetry snapshots are emitted once after each successful job as structured `event=telemetry_health`. They have no tenant/device/channel/token/readings identifiers. The three health distributions consume rollup snapshots only, avoiding maintenance's daily sample cadence in backlog alerts. Log-based histogram percentiles are bucket estimates, not exact count/age gauges; the source log remains exact. Default-row presence therefore alerts on a separate integer counter filtered by the exact numeric `default_rows>0` predicate, never on a percentile estimate around zero. Zero and missing row counts are excluded by the [numeric comparison filter](https://docs.cloud.google.com/logging/docs/view/logging-query-language); only positive snapshots increment this counter. These are operational starting thresholds, to tune from budgeted load measurements.
