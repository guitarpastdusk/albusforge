...
| `telemetry_maintenance_heartbeat` | Maintenance succeeded **and** its health query completed | No success in 25h, evaluated every 5m |
...
Daily maintenance uses PromQL's 25-hour lookback with five-minute evaluation. User-defined log-based metrics are explicitly limited to 25h, even though other metric types support longer histories, in [Cloud Monitoring PromQL alerting](https://docs.cloud.google.com/monitoring/alerts/using-promql). Native [metric absence](https://docs.cloud.google.com/monitoring/alerts/metric-absence) has a 23.5-hour maximum and would false-alarm on a daily schedule. The expression combines a zero-total check with `absent_over_time`: a log counter can emit zero values, and those must not count as successful maintenance. No prior series is unhealthy immediately when the policy is enabled; run a successful maintenance job and verify its point first. The one-hour grace covers daily startup/runtime/ingestion delay, not a guarantee of retention health. A completed job with backlog can still be unhealthy, hence separate queue/default policies.

The initial 26h policy was rejected by the Cloud Monitoring API during staging apply; local Prometheus expression tests do not enforce provider-specific metric-history limits. The corrected policy stays at the documented 25h limit with no offset or additional retest window.
