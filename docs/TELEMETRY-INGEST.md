...
| Cloud SQL | Direct private-IP PostgreSQL over direct VPC egress. `DB_HOST` is the private IP, `DB_PORT=5432`, `DB_SSL=require`; no connector, proxy, `INSTANCE_CONNECTION_NAME` or Cloud SQL Client IAM role needed for this connection path |
| DB credentials | `DB_NAME=albus`, `DB_USER=albus_app`, `DB_PASSWORD` from Secret Manager `db-app-password`; SQL application permissions, no migration DDL privileges |
...
Both environments use `createDb` / `dbConfigFromEnv` from `@albusforge/db`. `DB_SSL` defaults to `require`; cloudlink rejects `disable` when `K_SERVICE` identifies Cloud Run. The shared library uses encrypted TLS without CA verification (`rejectUnauthorized: false`), matching existing services and the Cloud SQL private-IP configuration; this does not claim certificate identity verification. Local development explicitly uses `DB_SSL=disable`. Only the SQL user/password authenticate the database session; cloudlink calls no Google connection API.
