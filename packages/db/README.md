...
| `src/client.ts` | `createDb(config)` returns `{ db, pool }`, a typed Drizzle instance over a `pg` pool. `createClientDb(client)` returns `{ db, close }` over one checked-out connection, for work holding a session resource (an advisory lock) that must not take a second connection. `await close({ withinMs })` ends the handle: it stops new queries, waits for the ones in flight, and rolls back, so the connection is clean for whoever uses it next — a statement abandoned mid-transaction would otherwise make the next `BEGIN` fail with 25P02. `withinMs` bounds the whole of that, rollback included, for a caller with a response deadline: past it nothing more is sent, and `close()` rejects with a `ClientDbCleanupError`. Pass that error to `client.release(error)` so the pool destroys the connection, which is what ends the work still on it |
